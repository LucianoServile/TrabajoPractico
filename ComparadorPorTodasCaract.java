...
public class ComparadorPorTodasCaract implements Comparator<Combatiente>{
	...
	public int compare(Combatiente c1, Combatiente c2){
		...
		try {
			if(mismaVelocidad && mismaFuerza && mismaResistencia) {
				System.out.println("If 1");
				retorno =  -Integer.compare(c1.getCaracteristica(Caracteristica.DESTREZA), c2.getCaracteristica(Caracteristica.DESTREZA));
			} else if(mismaVelocidad && mismaFuerza && !mismaResistencia) {
				System.out.println("If 2");
				retorno =  -Integer.compare(c1.getCaracteristica(Caracteristica.RESISTENCIA), c2.getCaracteristica(Caracteristica.RESISTENCIA));
			} else if( mismaVelocidad && !mismaFuerza) {
				System.out.println("If 3");
				retorno =  -Integer.compare(c1.getCaracteristica(Caracteristica.FUERZA), c2.getCaracteristica(Caracteristica.FUERZA));
			} else if(!mismaVelocidad){
				System.out.println("If 4");
				retorno =  -Integer.compare(c1.getCaracteristica(Caracteristica.VELOCIDAD), c2.getCaracteristica(Caracteristica.VELOCIDAD));
			} else {
